...
var
  ...
  SearchedInfo : TInfo;
begin
  if Dialogs.InputQuery('Поиск фильма по названию', 'Введите название', film) then
    if HashTable.Find(film, SearchedInfo) then
...
var
  film : string;
begin
  if Dialogs.InputQuery('Удаление записи', 'Введите название фильма', film) then
    if HashTable.Delete(film) then
...
begin
  if MessageDlg('Вы действительно хотите очистить таблицу?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    HashTable.Clear;
end;
